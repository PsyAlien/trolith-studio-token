...
// CHANGED: Import ITRI interface instead of StudioToken
import { ITRI } from "./interfaces/ITRI.sol";
import { ITaxProcessor } from "./interfaces/ITaxProcessor.sol";

contract TokenShop is Ownable {
    using SafeERC20 for IERC20;

    // CHANGED: Use ITRI instead of StudioToken
    ...
    // Tax logging (optional — works fine without it)
    ...
    constructor(
        // CHANGED: Accept ITRI instead of StudioToken
        ITRI token_,
        uint256 maxEthIn_,
        uint256 maxGenIn_,
    ...
    /// @dev Logs a tax event if TaxProcessor is configured. Never reverts.
    ///      gainOrLossSEK is 0 for now — backend calculates real SEK value later.
    ...
    // ---------------- SELL FLOWS (burn from shop balance) ----------------
    ...
    function sellToETH(uint256 genIn, uint256 minEthOut) external {
        ...
        // CHANGED: TRI burn takes (from, amount) instead of just (amount)
