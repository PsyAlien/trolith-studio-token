...
contract TokenShopComprehensiveTest is Test {
    ...
    function setUp() public {
        // Deploy TRI token (admin = this contract, gets all roles)
        token = new MockTRI();

        // Deploy shop (cast MockTRI to ITRI so Solidity accepts it)
        shop = new TokenShop(ITRI(address(token)), MAX_ETH_IN, MAX_GEN_IN, BUY_RATE_ETH, SELL_RATE_ETH);
    ...
    function test_sellETH_revertsAboveMaxGenIn() public {
        // Mint TRI directly to alice for this test
